...
namespace CSBC.Core.Data
{
    ...
    public class RepositoryFactories
    {
        ...
        /// </remarks>
        private IDictionary<Type, Func<DbContext, object>> GetCsbcFactories()
        {
            return new Dictionary<Type, Func<DbContext, object>>
                {
                   {typeof(IScheduleGameRepository), dbContext => new ScheduleGameRepository(dbContext)},
                   {typeof(IColorRepository), dbContext => new ColorRepository(dbContext)} //,
                   //{typeof(IHouseholdRepository), dbContext => new HouseholdRepository(dbContext)}
                };
        }
        ...
        {
            _repositoryFactories = GetCsbcFactories();
        }
